...
jQuery('.span3').hover(function() {
	...
	jQuery(this).find('.links').addClass('hide');
});

jQuery('a.sb-icon-search').click(function(e) {
	e.preventDefault();
	var searchInput = jQuery('.sb-search-input');
	var searchContainer = jQuery('.sb-search');
	
	if (searchInput.hasClass('sb-search-input-open')) {
		searchInput.removeClass('sb-search-input-open');
		searchContainer.removeClass('sb-search-open');
	} else {
		searchInput.addClass('sb-search-input-open');
		searchContainer.addClass('sb-search-open');
	}
});

jQuery(document).ready(function() {
	cart.get();
	jQuery('#popover-search').popover({ html: true, content: jQuery('#popover-search-form').html() });
});